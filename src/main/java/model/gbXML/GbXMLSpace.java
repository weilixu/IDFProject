...
public class GbXMLSpace {
	...
	// the coordinates under space boundary element is the middle line of the
	// building
	...
	public void translateSpace(Element element) {
		...
		if (conditionType == null) {
			...
			// TODO Warning - no condition type is found for this space. Set to
			// unconditioned
		}
		...
		if (name != null) {
			String tempSpaceName = name.getText();
			if (tempSpaceName == null) {
		...
		if (volume != null) {
			String unit = volume.getAttributeValue("unit");
			if (unit == null) {
				unit = volumeUnit;
			}
		...
		if (lppa != null) {
			String unit = lppa.getAttributeValue("unit");
			// buildsimhub - w/m2
			lightPowerPerArea = Double.parseDouble(lppa.getText())
					* GbXMLUnitConversion.powerPerAreaRate(unit, "WattPerSquareMeter");
		}

		Element eppa = element.getChild("EquipPowerPerArea", ns);
		if (eppa != null) {
			String unit = eppa.getAttributeValue("unit");
			// buildsimhub - w/m2
			equipPowerPerArea = Double.parseDouble(eppa.getText())
					* GbXMLUnitConversion.powerPerAreaRate(unit, "WattPerSquareMeter");
		}

		Element pn = element.getChild("PeopleNumber", ns);
		if (pn != null) {
			String unit = pn.getAttributeValue("unit");
			// buildsimhub - m2/people
			if (unit.equals("NumberOfPeople")) {

				Double numPl = NumUtil.readDouble(pn.getText(), 0.0);
				if (numPl == 0.0) {
					peopleNumber = 0.0;
				} else {
		...
		if (!peopleHeatGains.isEmpty()) {
			...
			for (int i = 0; i < peopleHeatGains.size(); i++) {
				...
				String unit = pgh.getAttributeValue("unit");
		...
		if (infiltrationFlowEle != null) {
			...
			if (blowerDoorTest != null) {
				String unit = blowerDoorTest.getAttributeValue("unit");
				if (unit.equals("ACH") || unit.equals("AirChangesPerHour")) {
					...
					infiltrationUnit = "m2";
					infiltrationFlow = Double.parseDouble(blowerDoorTest.getText())
							* GbXMLUnitConversion.lengthUnitConversionRate(unit, "SquareMeters");
				}
			}
		}

		// TODO re process to check the data - if these are not filled, then we should
		// call our database to retrieve these data
		...
		if (infiltrationFlow == null) {
			...
			// TODO Warning - the infiltration flow isnot specified in the gbXML file. Reset
			// to 0.4 ACH
		}
